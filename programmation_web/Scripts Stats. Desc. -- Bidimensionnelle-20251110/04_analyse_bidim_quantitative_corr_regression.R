    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("Aide", 
                 ...
                 fluidRow(
                   column(4, offset = 2, textOutput("corrHelp1")),
                   column(4, offset = 2, textOutput("corrHelp2"))
                 ),
                 fluidRow(
                 ...
                 fluidRow(
                   column(4, offset = 2, textOutput("corrHelp3")),
                   column(4, offset = 2, textOutput("corrHelp4"))
                 ),
                 fluidRow(
...
server <- function(input, output){
  ...
  dataHelp <- reactive({
    inFileHelp <- "../data_shiny_bidim/exempleNuages.csv"
  ...
  output$corrHelp1 <- renderText({
    x.var = dataHelp()[which(dataHelp()[, 1] == "X1"), 2]
    y.var = dataHelp()[which(dataHelp()[, 1] == "Y1"), 2]
    coeff.tmp <- cov(x.var, y.var)/(sd(x.var)*sd(y.var))
    paste("Coefficient de corrélation linéaire =", round(coeff.tmp,digits = 2),
          "(lien fort)")
  })
  output$corrHelp2 <- renderText({
  ...
  output$corrHelp3 <- renderText({
    x.var = dataHelp()[which(dataHelp()[, 1] == "X3"), 2]
    y.var = dataHelp()[which(dataHelp()[, 1] == "Y3"), 2]
    coeff.tmp <- cov(x.var, y.var)/(sd(x.var)*sd(y.var))
    paste("Coefficient de corrélation linéaire =", round(coeff.tmp,digits = 2),
          "(artéfact du aux outliers)")
  })
  output$corrHelp4 <- renderText({
  ...
  output$nuage1 <- renderPlot({
    ...
    # Droite de régression linéaire (y~x) 
    abline(lm(x.var~y.var), col="blue", lwd = 2)
  })
